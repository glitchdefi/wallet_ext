...
export const SelectedOption: React.FC<Props> = React.memo(
  ({ values, onNext, onCancel }) => {
    const [hasBackedUp, setHasBackedUp] = useState(values);

    return (
      <Flex flex={1} flexDirection="column" px="32px" pb="24px">
        <Box mt="32px">
          <Text>
            Once the current mnemonic phrase is removed from the Glitch wallet,
            all your accounts and funds still exists in the blockchain. As long
            as you still keep the mnemonic passphrase, you can always restore it
            in the future.
          </Text>
        </Box>
